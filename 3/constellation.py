...
class constellation:
    ...
    def estimate_pe(self, SNRdb, iterations):
        ...
        errors = 0
        for sent in random.choices(self.symbols, k=iterations, weights=self.probabilites):
            ...
            bias = np.matrix([[N0/2*np.log(1/p)] for p in self.probabilites])-self.symbols_energy/2
