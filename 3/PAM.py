...
class PAM(constellation):
    def __init__(self, M: int, pulse_energy: float, probabilites: List[int]):
        self.M = M
    ...
    def theorical_pe(self, SNR):
        return 2*(1-1/self.M)*Q(np.sqrt(6*np.log2(self.M)*(10**(SNR/10))/(self.M**2-1))-\
                np.sqrt((self.M**2-1)/(24*np.log2(self.M)*(10**(SNR/10))))*\
                np.log(self.probabilites[0]/self.probabilites[1]))
